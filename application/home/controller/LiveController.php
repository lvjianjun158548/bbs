...
class LiveController extends Base
{
    ...
    public function index()
    {
        ...
        $list = \app\home\model\Live::where('user_id', $user_id)->order('id desc')->select();

        return view('Live_controller/index', ['list' => $list]);
    }

    public function create()
    {
        return view('Live_controller/create');
    }
    ...
    public function save(Request $request)
    {
        ...
        $live = \app\home\model\Live::create($params, true);
        //关联商品
        $live_goods = [];
        foreach ($params['goods_links'] as $link) {
            ...
            $goods = \app\home\model\Goods::find($goods_id);
            if ($goods) {
                $row['live_id'] = $live->id;
        ...
        $live_goods_model = new \app\home\model\LiveGoods();
        $live_goods_model->saveAll($live_goods);
        $this->success('操作成功', 'home/live_controller/index');
    }
    ...
    {
        $info = \app\home\model\Live::find($id);
        if ($info) {
            $info['goods'] = \app\home\model\LiveGoods::where('live_id', $id)->select();
        }

        return view('Live_controller/read', ['info' => $info]);
    }
}
